...
const monsters = Array.from({ length: 18 }, (_, index) => `/monsters/${String(index + 1).padStart(2, '0')}.png`)
...
function Home({ onBuild, onPlay }: { onBuild: () => void; onPlay: () => void }) {
  return <main className="dashboard page">
    <section className="welcome"><div><span className="eyebrow">GOOD MORNING, NIC</span><h1>Make learning feel<br /><em>like play.</em></h1><p>Create a question set in minutes, then let the monsters loose.</p><div className="button-row"><button className="primary" onClick={onBuild}><span>＋</span> Create a set</button><button className="secondary" onClick={onPlay}>▶ Try the game</button></div></div>
      <div className="welcome-art" aria-hidden="true"><span className="orb orb--one" /><span className="orb orb--two" /><img className="hero-monster hero-monster--left" src="/monsters/03.png" /><img className="hero-monster hero-monster--main" src="/monsters/09.png" /><img className="hero-monster hero-monster--right" src="/monsters/15.png" /><span className="spark spark--one">✦</span><span className="spark spark--two">✦</span></div>
    </section>
    <section className="content-section"><div className="section-title"><div><span className="eyebrow">YOUR LIBRARY</span><h2>Question sets</h2></div><button className="text-button">View all →</button></div>
      <div className="set-grid">
        <article className="set-card set-card--feature" onClick={onPlay}><div className="subject-icon">½</div><span className="card-label">MATHEMATICS · YEAR 5</span><h3>Fraction Foundations</h3><p>12 questions · Played yesterday</p><div className="card-footer"><span className="tiny-monsters"><img src="/monsters/01.png" /><img src="/monsters/07.png" /><img src="/monsters/12.png" /></span><button>Play now</button></div></article>
...
function SetBuilder({ set, jsonText, message, onJsonChange, onImport, onSetChange, onDone, onBack }: { set: QuestionSet; jsonText: string; message: string; onJsonChange: (value: string) => void; onImport: () => void; onSetChange: (value: QuestionSet) => void; onDone: () => void; onBack: () => void }) {
  ...
  return <main className="builder page"><button className="back-button" onClick={onBack}>← Back to sets</button><div className="builder-heading"><div><span className="eyebrow">CREATE A SET</span><h1>Ready in a flash.</h1><p>Write questions yourself or paste a set made with AI.</p></div><img src="/monsters/06.png" /></div>
